...
public class AuthenticateUserController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (isAuthenticated) {
			...
			if (type.equals("edit")) {
				response.sendRedirect("/contact-database-app/EditContact?id="
						+ id);
			} else if(type.equals("delete")){
				response.sendRedirect("/contact-database-app/DeleteContact?id="
						+ id);
			}
			else{
				response.sendRedirect("/contact-database-app/AddContact");
			}
		} else {
			out.println("<script type=\"text/javascript\">");
			out.println("alert('Incorrect email id or password');");
			out.println("location='/contact-database-app/Authenticate?id=" + id
					+ "&type=" + type + "';");
			out.println("</script>");
		}
	}
